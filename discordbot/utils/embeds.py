...
def build_dashboard_embed(df_live, status_map, all_pannes, heure_actuelle,
    ...
    for park_name, lands in parks_data.items():
        ...
        if lines:
            # Discord limite les fields à 1024 chars — on découpe si besoin
            chunk = ""
            for line in lines:
...
def build_ride_embed(ride_name, status, detail, wait_time, pannes_30j, emoji="🎡"):
    s = STATUS_MAP.get(status, STATUS_MAP["INCONNU"])
    embed = discord.Embed(
        title=f"{emoji} {ride_name}",
        color=s["color"]
    )
    ...
    if pannes_30j:
        nb    = len(pannes_30j)
        total = sum(p["duree"] for p in pannes_30j if p["duree"])
...
def build_notif_embed(ride_name, old_status, new_status, detail, emoji="🎡"):
