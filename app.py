    ...
    if selected_options:
        ...
        if sort_mode == "⏳ Temps d'Attente":
            ...
            def is_incident(r):
                ...
                i = status_map.get(r, {})
                h_f_c = ANTICIPATED_CLOSINGS.get(r) or (DAW_CLOSING if any(a.lower() in r.lower() for a in RIDES_DAW) else DLP_CLOSING)
...
if not df_pannes_brutes.empty:
    ...
    for _, p in flux.iterrows():
        ...
        if not h_f_p:
            render_ride_card(r_n, f"En panne à {d_p.strftime('%H:%M')}", "101", "bg-orange", "card-orange", "INTERRUPTION")
        else:
            render_ride_card(r_n, f"Réouvert à {h_f_p}", "OK", "bg-green", "card-green", "REOUVERTURE")
