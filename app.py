...
if not df_raw.empty:
    ...
    if not df.empty:
        ...
        # --- LOGIQUE DES RACCOURCIS (MULTI-ALIAS) ---
        st.write("---")
        sc = st.text_input("Raccourci land ou parc (ex: `*MS`, `*WOF`, `*FRONTIER`...)", placeholder="Tape ici...")
        
        # Récupération des favoris actuels dans l'URL
        current_selection = st.query_params.get_all("fav")

        # Traitement des nouveaux raccourcis
        if sc == "*101":
            current_selection = [p['ride'] for p in all_pannes if p['statut'] == "EN_COURS"]
        elif sc == "*102":
            current_selection = list(set([p['ride'] for p in all_pannes]))
        elif sc.startswith("*"):
            # Cette fonction dans emojis.py doit gérer tes nouveaux alias (*MS, *WOF, etc.)
            shortcut_selection = get_rides_by_zone(sc.upper(), toutes_attractions)
        ...
        if not selected_options:
            st.info(f"👆 Utilise les raccourcis land. (Reset quotidien : 02h00)")
        else:
        ...
        st.warning(f"😴 Mode nuit / Maintenance. (Reset à {debut_journee.strftime('%H:%M')})")
else:
