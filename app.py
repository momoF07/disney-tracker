...
st_autorefresh(interval=60000, key="datarefresh")[cite: 5]
...
with col_info1:
    ...
    for p_id in ["DLP", "DAW"][cite: 5]:
        s = next((item for item in sched_data if item["park_id"] == p_id), None)[cite: 5]
        if s:
            o = pd.to_datetime(s['opening_time']).astimezone(paris_tz).strftime("%H:%M")[cite: 5]
            c = pd.to_datetime(s['closing_time']).astimezone(paris_tz).strftime("%H:%M")[cite: 5]
            label = "🏰 Disneyland" if p_id == "DLP" else "🎬 Adventure"[cite: 5]
            st.write(f"**{label}** : {o} - {c}")[cite: 5]

with col_info3:
    st.markdown("#### 🎭 Shows (Prochaines 2h)")
    if upcoming_shows[cite: 5]:
        for s in upcoming_shows[:3]:
            t_show = pd.to_datetime(s['start_time']).astimezone(paris_tz).strftime("%H:%M")[cite: 5]
            st.write(f"⌚ **{t_show}** - {s['show_name']}")[cite: 5]
    else:
...
def render_park(park_key):
    # Initialisation de la variable pour éviter l'UnboundLocalError[cite: 5]
    all_rides = [r for land in cfg.PARKS_DATA[park_key].values() for r in land]
    
    if sort_mode == "⏳ Attente":
        all_rides = sorted(all_rides, key=lambda x: live_data.get(x, {}).get('wait_time', 0), reverse=True)[cite: 5]
    elif sort_mode == "⚠️ Incidents":
        # Tri par statut (False/0 passe avant True/1) puis nom[cite: 5]
        all_rides = sorted(all_rides, key=lambda x: (live_data.get(x, {}).get('is_open', True), x))[cite: 5]
    else:
        all_rides = sorted(all_rides)[cite: 5]
            ...
            with cols[i % 4]:
                with st.container(border=True):
                    if data:
                        ...
                        st.markdown(f'<span class="status-pill {pill}">{"OUVERT" if is_open else "INCIDENT"}</span>', unsafe_allow_html=True)[cite: 5]
                        st.markdown(f"**{cfg.get_emoji(ride_name)} {ride_name}**")[cite: 5]
                        if is_open:
                            wait = data['wait_time']
                            c_w, c_p = st.columns([2, 1])
                            c_w.markdown(f"### {wait} min")[cite: 5]
                            with c_p:
                                with st.popover("📈"):
                                    h_df = get_ride_history_24h(supabase, ride_name)[cite: 3]
...
with tabs[0]: render_park("Disneyland Park")[cite: 5]
with tabs[1]: render_park("Disney Adventure World")[cite: 5]
...
if logs:
    for l in logs:
        t_log = pd.to_datetime(l['start_time']).astimezone(paris_tz).strftime("%H:%M")[cite: 5]
        event = "⚠️ Panne" if l['end_time'] is None else "✅ Réouverture"[cite: 5]
        color = "#e74c3c" if l['end_time'] is None else "#2ecc71"[cite: 5]
