...
with st.sidebar.expander("🔐 Admin Panel"):
    ...
    if admin_pass == st.secrets.get("ADMIN_PASSWORD", "disney123"):
        ...
        if sim_mode and not df_live.empty:
            ...
            df_live['wait_time'] = [random.randint(5, 75) for _ in range(len(df_live))]
            df_live['is_open'] = [random.choice([True, True, True, False]) for _ in range(len(df_live))]
            for r in status_map: status_map[r] = True
