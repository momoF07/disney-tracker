...
# On retire 2h pour parler à Supabase (UTC)
...
if not df.empty:
    ...
    if not df.empty:
        ...
        for p_f in flux:
            if p_f['statut'] == "EN_COURS": st.error(f"🔴 {p_f['ride']} ({p_f['debut'].strftime('%H:%M')})")
            else: st.info(f"✅ {p_f['ride']} rouvert à {p_f['fin'].strftime('%H:%M')} ({p_f['duree']} min)")
    else:
