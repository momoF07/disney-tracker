...
# --- LIGNE 1 : FILTRES GLOBAUX & ÉTATS OPÉRATIONNELS ---
...
with c4:
    if st.button("⚠️ 101", use_container_width=True, help="En panne actuellement"):
        ...
        st.rerun()
with c5:
    if st.button("⏪ 102", use_container_width=True, help="Historique des interruptions du jour"):
        # Liste les attractions qui apparaissent dans la table des pannes aujourd'hui
        rides_with_incidents = list(set([p['ride'] for p in all_pannes]))
        st.query_params["fav"] = rides_with_incidents
        st.rerun()
with c6:
    if st.button("🏁 FERMÉ", use_container_width=True, help="Attractions fermées pour la nuit"):
        # On réutilise la logique h_f définie plus bas ou on filtre par data['is_open']
        closed_rides = []
        for r in options:
            # Calcul rapide de h_f pour chaque attraction
            ...
            if heure_actuelle >= h_f_check:
                closed_rides.append(r)
        st.query_params["fav"] = closed_rides
        st.rerun()

# --- LIGNE 2 : ZONES DISNEYLAND PARK ---
