...
if not df_raw.empty:
    ...
    if not df.empty:
        ...
        with col_help:
            with st.popover("❓"):
                st.markdown("**Index des codes :**\n`*ALL`, `*DLP`, `*DAW`, `*FANTASY`, `*DISCO`, `*CAMPUS`...")
        ...
        if selected_options:
            ...
            for ride in selected_options:
                ...
                if not ride_df.empty:
                    ...
                    if tous_fermes_globalement and maintenant.hour >= 18:
                        c1.error("🔴 PARC FERMÉ")
                        c2.metric("Attente", "- - -")
                    
                    # 2. SI L'ATTRACTION N'A JAMAIS OUVERT (MATIN)
                    ...
                    with st.expander("📜 Historique des pannes"):
                        ...
                        else: st.write("✅ Aucune panne aujourd'hui.")
                    st.divider()
    ...
    else: st.warning("😴 Maintenance nocturne (Données réinitialisées à 2h).")
